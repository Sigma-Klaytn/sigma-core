...
contract FeeDistributor is Ownable {
    ...
    function setAddresses(ISIGLocker _tokenLocker) external onlyOwner {
        ...
        returns (uint256, StreamData memory)
    {
        ...
        for (uint256 i = lastClaimWeek; i < claimableWeek; i++) {
            (uint256 userWeight, uint256 totalWeight) = tokenLocker
    ...
    function _buildStreamData(
        ...
        (uint256 userWeight, uint256 totalWeight) = tokenLocker.weeklyWeight(
            _user,
            _week
        );
        uint256 amount;
