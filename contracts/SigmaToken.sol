...
contract OwnerRole is MinterRole, PauserRole {
    ...
    constructor(address payable owner) MinterRole(owner) PauserRole(owner) {
        _owner = owner;
    }
...
contract KIP7 is IKIP7, KIP13 {
    ...
    function _checkOnKIP7Received(
        ...
        if (
            retval.length != 0 && abi.decode(retval, (bytes4)) == _KIP7_RECEIVED
        ) {
            return true;
        }
    ...
    OwnerRole
{
    ...
    function transferFrom(
        ...
        address to,
        uint256 value
    ) public virtual override(KIP7, KIP7Pausable) returns (bool) {
