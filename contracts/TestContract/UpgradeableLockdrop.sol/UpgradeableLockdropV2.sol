...
contract UpgradeableLockdropV2 is
    Initializable,
    OwnableUpgradeable,
    UUPSUpgradeable,
    ...
    PausableUpgradeable
{
    ...
    /* ========== STATE VARIABLES ========== */
    IERC20Upgradeable public SIG; // reward token.
    IERC20Upgradeable public KSP; // deposit token
    ...
    uint256 public constant TOTAL_SIG_SUPPLY = 2500000000000000000000000;

    uint256 public constant LOCK_1_MONTHS = 2628000;
    uint256 public constant LOCK_3_MONTHS = 7884000;
    uint256 public constant LOCK_6_MONTHS = 15770000;
    uint256 public constant LOCK_9_MONTHS = 23650000;
    uint256 public constant LOCK_12_MONTHS = 31540000;
     ...
     */
    function deposit(uint256 _amount, uint256 _lockMonth)
        external
        nonReentrant
        whenNotPaused
    {
    ...
    function setLockMonth(uint256 _lockMonth) external whenNotPaused {
        require(block.timestamp > phase1StartTs, "Phase 1 did not start yet.");
        require(
    ...
    */
    function claimSIGTokens() external nonReentrant whenNotPaused {
        require(
            block.timestamp > phase2EndTs,
    ...
    /**
        @notice Withdraw pro-rata allocated SIG tokens, in vested manner. 
    */
    function withdrawLPTokens() external nonReentrant whenNotPaused {
        require(
            block.timestamp > phase2EndTs,
            "You can't withdraw tokens before phase 2 ends."
        );
        ...
        lpToken.safeTransfer(msg.sender, amount / 2);
        emit LPTokenClaimed(msg.sender, amount / 2);
    }
    ...
    */
    ...
    function adminRewardTokenWithdraw(address _token) external onlyOwner {
        ...
        uint256 balanceOfToken = IERC20Upgradeable(_token).balanceOf(
            address(this)
        );
        require(balanceOfToken > 0, "There is no withdrawable amount of KSP");
        IERC20Upgradeable(_token).transfer(receiver, balanceOfToken);
