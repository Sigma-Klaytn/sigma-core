...
contract Vault is Ownable {
    ...
    function lockTokens(IERC20 _token, address _withdrawer, uint256 _amount,uint256 _unlockTimestamp) external returns (uint256 _id) {
        ...
        require(_token.allowance(msg.sender, address(this))>=_amount,'Approve token first');
    ...
    function getDepositsByTokenAddress(address addr) view external returns (uint256[] memory) {
        return depositsByTokenAddress[addr];
    }
