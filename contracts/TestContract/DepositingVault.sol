...
contract DepositingVault is Ownable {
    ...
    event MINT_TOKEN(uint256 amount);
    event LOCKED_TOKEN(uint256 id, uint256 amount, uint256 unlockTimestamp);
    event Withdraw(uint256 id);

    function mintERC20Token(address _token, uint256 _amount)
        external
        returns (uint256)
    {
    ...
    function lockTokens(
        ...
        uint256 id = IVault(_vault).lockTokens(
            IERC20(_token),
            _withdrawer,
            _amount,
            _unlockTimestamp
    ...
    function getVaultsByWithdrawer(address _vault, address _withdrawer)
        external
        view
        returns (uint256[] memory)
    {
        uint256[] memory ids = IVault(_vault).getVaultsByWithdrawer(
            _withdrawer
        );
        return ids;
    }
