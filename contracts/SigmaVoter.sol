...
import "./interfaces/sigma/ISIGLocker.sol";

// import "./interfaces/solidly/IBaseV1Voter.sol";

contract SigmaVoter is Ownable {
    uint256 public tokenID;

    // IBaseV1Voter public immutable solidlyVoter;

    ISIGLocker public tokenLocker;
    ...
    function getPoolProtectionData(address user)
        external
        view
        returns (uint256 lastUpdate, address[2] memory pools)
    {
        return (
            poolProtectionData[user].lastUpdate,
            poolProtectionData[user].pools
        );
    }
        ...
        external
    {
        ...
        for (uint256 x = 0; x < _pools.length; x++) {
            ...
            if (_weight < 0) {
                require(
                    poolProtectionCount[_pool] == 0,
                    "Pool is protected from negative votes"
                );
            }
