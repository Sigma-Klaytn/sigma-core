...
contract SIGLocker is Ownable {
    ...
    constructor(uint256 _maxLockWeeks, address _SIG) {
    ...
    /**
        ...
        @param _amount Amount of SIG to lock. This balance transfered from the caller.
    ...
    function lock(
        ...
        require(_weeks > 0, "Minimum lock week is 1");
    ...
    /**
        ...
        @param _amount Amount of SIG to extend the lock for. When the value given equals
    ...
    function withdrawExitStream() external returns (bool) {
        StreamData storage stream = exitStream[msg.sender];
        uint256 amount;
        //TODO: require stream.start should be bigger than 0
        if (stream.start > 0) {
    ...
    /**
        @notice Get the amount of SIG in expired locks that is
    ...
    /**
        @notice Get the amount of SIG available to withdraw
    ...
    function setMaxLockWeek(uint256 week) external onlyOwner {
        require(week > 0, "MAX_LOCK_WEEKS should be bigger than 0");
        MAX_LOCK_WEEKS = week;
    }
}
