...
//     // Solidly Voter : 소스코드 -> https://github.com/solidlyexchange/solidly/blob/master/contracts/BaseV1-voter.sol
//     IBaseV1Voter public immutable solidlyVoter;

//     // solidex contracts
//     ISolidexToken public SEX;
//     // SOLIDsex : SOLID 는 1:1 교환비율
//     IVeDepositor public SOLIDsex;
//     //claim fees and bribes for locking Sex
...
//     }
//     // Pool 예시 : USDC/MIM
//     // Pool 주소 : 	0xbcab7d083Cf6a01e0DdA9ed7F8a02b47d125e682
//     // 내 주소 : 0xaA4879742Fb105b59FE589e321f5243dFb72a24E

//     // pool -> gauge
//     // 0xC009BC33201A85800b3593A40a178521a8e60a02 (Solidly Gauge Smart Contract)
//     mapping(address => address) public gaugeForPool;

//     // pool -> bribe
//     //0xE8f0802FefAeA2EbF22e518Efa597477e6742C4F (Solidly Bribe Smart Contract : https://github.com/solidlyexchange/solidly/blob/master/contracts/BaseV1-bribes.sol )
//     mapping(address => address) public bribeForPool;

//     // pool -> solidex deposit token
//     //0x895B8AF7A0909A9268fB11c6E5e6DbDE2f04299B (Solidex sAMM-USDC/MIM Deposit 이며 LpDepositToken.sol <- 얘 참고)
//     mapping(address => address) public tokenForPool;

//     // user -> pool -> deposit amount
//     // 10181421925679 wei = 0.000010181421925679 eth(ftm)
//     mapping(address => mapping(address => uint256)) public userBalances;

//     // pool -> total deposit amount
//     // 79.606401671636690912 LP
//     mapping(address => uint256) public totalBalances;

//     // pool -> integrals
//     //[ rewardIntegral method Response ]
//         // solid   uint256 :  2930646288724693650274 (2930.646288724693650274 solid)
//         // sex   uint256 :  696628464837455993813 (696.628464837455993813 sex)
//     mapping(address => Amounts) public rewardIntegral;

//     // 아래 두 개 함수는 pendingRewards 로 확인해야함.
//     // SOLID Reward : 50452292236513 (0.000050452292236513)
//     // SEX Reward : 11992748160525 (0.000011992748160525)
//     // user -> pool -> integrals
...
//         Solidex 문서를 참고해보면
//         1)  Earn SOLID: Boosted Solidly emissions from the protocol's veSOLID (earned as vanilla SOLID)
//         2)  Earn SEX: 1 SEX for every 4.2069 SOLID earned (as vanilla SEX)
//         3)  Pay fees: 15% performance fee on SOLID earned
//      */
...
//         pedning = new Amounts[](pools.length);
//         for (uint256 i = 0 ; i < pools.length ; i++) {
//             address pool = pools[i];
//             //가져올 수 있는 양을 넣음
//             pending[i] = claimable[account][pool];
...
//                 // 1)  Earn SOLID: Boosted Solidly emissions from the protocol's veSOLID
//                 integral.solid += 1e18 * delta / total;
...
//             //풀에 할당된 솔리드의 양보다 유저에게 할당된 솔리드의 양이 작다면
//             if(integralFor.solid < integral.solid) {
...
//         if (gauge == address(0)) {
//             //solidly의 gauge 를 가져옴
//             gauge = solidlyVoter.gauges(pool);

//             //만약에 gauge가 없으면 gauge 를 만듬.
//             if (gauge == address(0)) {
...
//         uint256 total
//     ) internal {
//         //현재 pool 에 할 당된 reward 의 양
//         Amounts memory integral = rewardIntegral[pool];
//         if (total > 0) {
//             // 이 컨트랙트의 solid 양
//             uint256 delta = SOLID.balanceOf(address(this));

//             //solid 의 주소를 rewards에 넣음. 아래 함수를 실행하기 위해서
