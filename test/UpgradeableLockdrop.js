const { makeErc20Token, UpgradeableLockdrop, MockERC20, makeUpgradeableLockdrop, makeUpgradeableLockdropV2, UpgradeableLockdropV2, makeUUPSProxy, UUPSProxy } = require('./Utils/Sigma');
...
contract('Lockdrop', function (accounts) {
    ...
    describe('Test : 0.SetInitialInfo , 1.Deposit requirements', () => {
        ...
        before(async () => {
            ...
            LockdropImpl = await makeUpgradeableLockdrop()
            LockdropProxy = await makeUUPSProxy(LockdropImpl.address, "0x")
            Lockdrop = await UpgradeableLockdrop.at(LockdropProxy.address);

            await Lockdrop.initialize()
    ...
    describe('Test : 7. Claim SIG Tokens (User) ~ 8. Withdraw LP Tokens after Lock period', () => {
        ...
        it('8. Withdraw LP Tokens after Lock period', async () => {
            ...
            const totalLPTokenAmount2 = await Lockdrop.totalLPTokenSupply();
            console.log('totalLPTokenSupply 22222: ', totalLPTokenAmount2.toString())


            // [IMPORTANT] Upgrade test

            let LockdropImpl2 = await makeUpgradeableLockdropV2()
            await Lockdrop.upgradeTo(LockdropImpl2.address)
            Lockdrop = await UpgradeableLockdropV2.at(LockdropProxy.address);

            userInfo = await Lockdrop.depositOf(userC)
            userAmount = userInfo[0];
            console.log('user amount v2 : ', userAmount.toString())
            const totalLPTokenAmount3 = await Lockdrop.totalLPTokenSupply();
            const totalDeposit3 = await Lockdrop.totalDeposit();
            console.log('totalLPTokenSupply : ', totalLPTokenAmount3.toString())
            let expectedLpAmount2 = totalLPTokenAmount.mul(userAmount).div(totalDeposit3)
            console.log('user C expeceted LP amount : ', expectedLpAmount2.toString())

            await time.increase(time.duration.seconds(LOCK_1_MONTHS * 10)) // after two months 

            receipt = await Lockdrop.withdrawLPTokens({ from: userC })
            expectEvent(receipt, 'LPTokenClaimed', {
                user: userC,
                amount: expectedLpAmount2.div(new BN(2))
            })

            await lpToken.approve(Lockdrop.address, MAX_UINT_256, { from: userC })
            await Lockdrop.depositLPToken(new BN(1), { from: userC })
