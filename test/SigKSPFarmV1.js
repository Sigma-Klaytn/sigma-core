...
contract('sigKSPFarmV1', function (accounts) {
    let root = accounts[0];
    ...
    describe('Test : 0.SetInitialInfo', () => {
        ...
        before(async () => {
            ...
            sigKSPFarmImpl = await makeSigKSPFarmV1();
            sigKSPFarmProxy = await makeUUPSProxy(sigKSPFarmImpl.address, "0x");

            sigKSPFarm = await SigKSPFarmV1.at(sigKSPFarmProxy.address);
        ...
        it('Test : 1. Fund', async () => {
            ...
            await sigToken.approve(sigKSPFarm.address, MAX_INT256, { from: root });
        ...
        it('Test : 6. Upgrade Test', async () => {
            ...
            let sigKSPFarmV2_TestImpl = await makeSigKSPFarmV2_Test();
            await sigKSPFarm.upgradeTo(sigKSPFarmV2_TestImpl.address)
            sigKSPFarm = await SigKSPFarmV2_Test.at(sigKSPFarmProxy.address);

            await expectRevert(sigKSPFarm.deposit(new BN(10000), { from: userA }), "Deposit sigKSP amount should be bigger than 1")

            await sigKSPFarm.deposit(bnMantissa(1), { from: userA })
